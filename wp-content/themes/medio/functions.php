...
function ocdi_import_files() {
    ...
    $src_path = 'http://themeton.com/oneclick/'.wp_get_theme()->template.'/';
    return array(
        array(
            'import_file_name'           => 'medio',
            'categories'                 => array( 'Medio' ),
            'import_file_url'            => $src_path .'content.xml',
            'import_widget_file_url'     => $src_path .'widgets.wie',
            'import_redux'               => array(
                array(
                    'file_url'    => $src_path .'theme-options.json',
                    'option_name' => 'themeton_redux',
                ),
            ),
            'import_preview_image_url'   => $src_path .'screen-image.png',
            'import_notice'              => __( 'After you import this demo, you will have to setup the slider separately.', 'medio' ),
            'preview_url'                => 'http://next.themeton.com/medio',
            'src_path'                   => $src_path
        ),
...
function ocdi_after_import( $selected_import ) {
    ...
    set_theme_mod( 'logo_img', get_template_directory_uri() . '/images/logo.png' );
    ...
    // Revolution slider import attempt
    if ( class_exists( 'RevSlider' ) ) {

        $rev_sliders_array = array(
            'medio-home'    => $selected_import['src_path'] .'slider-home.zip',
            'testimonial'   => $selected_import['src_path'] .'slider-testimonial.zip'
        );
        foreach ($rev_sliders_array as $alias => $slider_zip) {
