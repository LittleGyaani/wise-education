...
function redux_css_compiler($post_id,$type)
{
    if ($type == 'header') {
        ...
        // Primary menu active background color
        $mbgcolor = get_post_meta($post_id,'header-color-menuactive',true);
        $menu_bgcolor = isset($mbgcolor) ? $mbgcolor : '';
        
        if (isset($menu_bgcolor) && $menu_bgcolor!='') {
            $css .= '.themeton-menu > li:hover > a, .themeton-menu > li.current-menu-item > a, .themeton-menu > li.current_page_item > a, .themeton-menu > li.current-menu-ancestor > a {';
            $css .= 'background:'.$menu_bgcolor.'!important;';
            $css .= '}';
        }
        // Primary menu active border color
        $mbordercolor = get_post_meta($post_id,'header-color-menuactiveborder',true);
        $menu_bordercolor = isset($mbordercolor) ? $mbordercolor : '';
        if (isset($menu_bordercolor) && $menu_bordercolor!='') {
            $css .= '.themeton-menu > li:hover > a, .themeton-menu > li.current-menu-item > a, .themeton-menu > li.current_page_item > a, .themeton-menu > li.current-menu-ancestor > a {';
            $css .= 'border-right-color:'.$menu_bordercolor.'!important;';
            $css .= '}';
        }
