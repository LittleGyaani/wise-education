...
// Start class
if ( ! class_exists( 'Bauer_Typography' ) ) {
	class Bauer_Typography {
		public function __construct() {
			// Customizer actions
			add_action( 'customize_register', array( 'Bauer_Typography' , 'register' ), 40 );

			// Loads Google fonts
			add_action( 'wp_enqueue_scripts', array( 'Bauer_Typography', 'load_fonts' ) );
		
			// CSS output
			if ( is_customize_preview() ) {
				add_action( 'customize_preview_init', array( 'Bauer_Typography', 'customize_preview_init' ) );
				add_action( 'wp_head', array( 'Bauer_Typography', 'live_preview_styles' ), 999 );
			} else {
				add_filter( 'bauer_custom_colors_css', array( 'Bauer_Typography', 'head_css' ), 99 );
			}
		}
		...
		public static function elements() {
			// Set default font
			$body_default = 'Poppins';

			// Return settings
			$array = apply_filters( 'bauer_typography_settings', array(
				'body' => array(
					'label' => esc_html__( 'Body', 'bauer' ),
					'target' => 'body',
					'defaults' => array(
				...
				'headings' => array(
					'label' => esc_html__( 'Headings', 'bauer' ),
				...
				'main_menu' => array(
					'label' => esc_html__( 'Main Menu', 'bauer' ),
					'target' => '#main-nav > ul > li > a',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'main_menu_dropdown' => array(
					'label' => esc_html__( 'Main Menu: Dropdowns', 'bauer' ),
				...
				'featured_title' => array(
					'label' => esc_html__( 'Featured Title', 'bauer' ),
					'target' => '#featured-title .main-title',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'bauer_cac_has_featured_title_heading',
				),
				'featured_subtitle' => array(
					'label' => esc_html__( 'Featured Sub-Title', 'bauer' ),
					'target' => '#featured-title .sub-title',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'bauer_cac_has_featured_title_heading',
				),
				'breadcrumbs' => array(
					'label' => esc_html__( 'Breadcrumbs', 'bauer' ),
					'target' => '#featured-title #breadcrumbs',
					'exclude' => array( 'font-color', 'line-height' ),
					'active_callback' => 'bauer_cac_has_featured_title_breadcrumbs',
				),
				'blog_post_title' => array(
					'label' => esc_html__( 'Blog Post Title', 'bauer' ),
					'target' => '.hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'blog_single_post_title' => array(
					'label' => esc_html__( 'Blog Single Post Title', 'bauer' ),
					'target' => '.is-single-post .hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'theme_button' => array(
					'label' => esc_html__( 'Buttons', 'bauer' ),
					'target' => '.bauer-button, button, input[type="button"], input[type="reset"], input[type="submit"], .footer-promotion .promo-btn, .products li .product-info .button, .products li .product-info .added_to_cart, .woocommerce-page .wc-proceed-to-checkout .button, .woocommerce-page #payment #place_order',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'theme_pagination' => array(
					'label' => esc_html__( 'Pagination', 'bauer' ),
					'target' => '.bauer-pagination, .woocommerce-pagination',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'sidebar_widget_title' => array(
					'label' => esc_html__( 'Sidebar Widget Title', 'bauer' ),
					'target' => '#sidebar .widget .widget-title',
				),
				'footer_widget_title' => array(
					'label' => esc_html__( 'Footer Widget Title', 'bauer' ),
					'target' => '#footer-widgets .widget .widget-title',
					'exclude' => array( 'font-color' ),
				),
				'entry_h1' => array(
					'label' => esc_html__( 'H1', 'bauer' ),
					'target' => 'h1',
				),
				'entry_h2' => array(
					'label' => esc_html__( 'H2', 'bauer' ),
					'target' => 'h2',
				),
				'entry_h3' => array(
					'label' => esc_html__( 'H3', 'bauer' ),
					'target' => 'h3',
				),
				'entry_h4' => array(
					'label' => esc_html__( 'H4', 'bauer' ),
					'target' => 'h4',
				),
				'copyright' => array(
		...
		public static function customize_preview_init() {
			wp_enqueue_script( 'bauer-typography-customize-preview',
				get_template_directory_uri() .'/framework/customizer/typography-customize.js',
				array( 'customize-preview' ),
				'1.0.0',
				true
			);
			wp_localize_script( 'bauer-typography-customize-preview', 'bauer', array(
				'googleFontsUrl' => bauer_get_google_fonts_url()
			) );
		}
		...
		public static function register ( $wp_customize ) {
			...
			// Add General Panel
			$wp_customize->add_panel( 'bauer_typography', array(
				'priority' => 142,
				'capability' => 'edit_theme_options',
				'title' => esc_html__( 'Typography', 'bauer' ),
			) );

			// Add General Tab
			$wp_customize->add_section( 'bauer_typography_general' , array(
				'title' => esc_html__( 'General', 'bauer' ),
				'priority' => 1,
				'panel' => 'bauer_typography',
			) );
			...
			$wp_customize->add_control( new Bauer_Customize_Multicheck_Control( $wp_customize, 'google_font_subsets', array(
				'label' => esc_html__( 'Font Subsets', 'bauer' ),
				'section' => 'bauer_typography_general',
			...
			foreach( $elements as $element => $array ) {
				...
				if ( $label ) {
					...
					// Define Section
					$wp_customize->add_section( 'bauer_typography_'. $element , array(
						'title' => $label,
						'priority' => $count,
						'panel' => 'bauer_typography',
					...
					if ( in_array( 'font-family', $attributes ) ) {
						...
						// Add Control
						$wp_customize->add_control( new Bauer_Fonts_Dropdown_Custom_Control( $wp_customize, $element .'_typography[font-family]', array(
								'label' => esc_html__( 'Font Family', 'bauer' ),
								'section' => 'bauer_typography_'. $element,
					...
					if ( in_array( 'font-weight', $attributes ) ) {
						$wp_customize->add_setting( $element .'_typography[font-weight]', array(
							'type' => 'theme_mod',
							'description' => esc_html__( 'Note: Not all Fonts support every font weight style.', 'bauer' ),
							'sanitize_callback' => false,
							'transport' => $transport,
						) );
						$wp_customize->add_control( $element .'_typography[font-weight]', array(
							'label' => esc_html__( 'Font Weight', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
							...
							'choices' => array(
								...
								'100' => esc_html__( 'Extra Light: 100', 'bauer' ),
								'200' => esc_html__( 'Light: 200', 'bauer' ),
								'300' => esc_html__( 'Book: 300', 'bauer' ),
								'400' => esc_html__( 'Normal: 400', 'bauer' ),
								'500' => esc_html__( 'Medium: 500', 'bauer' ),
								'600' => esc_html__( 'Semibold: 600', 'bauer' ),
								'700' => esc_html__( 'Bold: 700', 'bauer' ),
								'800' => esc_html__( 'Extra Bold: 800', 'bauer' ),
								'900' => esc_html__( 'Black: 900', 'bauer' ),
							),
							'description' => esc_html__( 'Important: Not all fonts support every font-weight.', 'bauer' ),
						) );
					}
					...
					if ( in_array( 'font-style', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-style]', array(
							'label' => esc_html__( 'Font Style', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'bauer' ),
								'normal' => esc_html__( 'Normal', 'bauer' ),
								'italic' => esc_html__( 'Italic', 'bauer' ),
							),
						) );
					}
					...
					if ( in_array( 'text-transform', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[text-transform]', array(
							'label' => esc_html__( 'Text Transform', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'bauer' ),
								'capitalize' => esc_html__( 'Capitalize', 'bauer' ),
								'lowercase' => esc_html__( 'Lowercase', 'bauer' ),
								'uppercase' => esc_html__( 'Uppercase', 'bauer' ),
							),
						) );
					}
					...
					if ( in_array( 'font-size', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-size]', array(
							'label' => esc_html__( 'Font Size', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
							'settings' => $element .'_typography[font-size]',
							'priority' => 5,
							'type' => 'text',
							'description' => esc_html__( 'Value in px or em.', 'bauer' ),
							'active_callback' => $active_callback,
						) );
					}
					...
					if ( in_array( 'font-color', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $element .'_typography_color', array(
							'label' => esc_html__( 'Font Color', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
					...
					if ( in_array( 'line-height', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[line-height]',
							array(
								'label' => esc_html__( 'Line Height', 'bauer' ),
								'section' => 'bauer_typography_'. $element,
					...
					if ( in_array( 'letter-spacing', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $element .'_typography_letter_spacing', array(
							'label' => esc_html__( 'Letter Spacing', 'bauer' ),
							'section' => 'bauer_typography_'. $element,
		...
		public static function loop( $return = 'css' ) {
			...
			foreach( $elements as $element => $array ) {
				...
				$get_mod = bauer_get_mod( $element .'_typography' );
				...
				foreach ( $attributes as $attribute ) {
					...
					if ( $val ) {
						...
						// Sanitize data
						$val = ( 'font-size' == $attribute ) ? bauer_sanitize_data( $val, 'font_size' ) : $val;
						$val = ( 'letter-spacing' == $attribute ) ? bauer_sanitize_data( $val, 'px' ) : $val;
		...
		public static function load_fonts() {
			...
			if ( ! empty( $fonts ) && is_array( $fonts ) ) {
				foreach ( $fonts as $font ) {
					bauer_enqueue_google_font( $font );
				}
			}
