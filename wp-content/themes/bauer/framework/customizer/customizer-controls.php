...
class Bauer_Customizer_Hr_Control extends WP_Customize_Control {
	public $type = 'hr';
...
class Bauer_Customizer_Heading_Control extends WP_Customize_Control {
	public $type = 'bauer-heading';
	public function render_content() {
		echo '<span class="bauer-customizer-heading">' . esc_html( $this->label ) . '</span>';
	}
}

// Textarea control for the customizer
class Bauer_Customizer_Textarea_Control extends WP_Customize_Control {
	public $type = 'bauer_textarea';
	public $rows = '3';
	public function render_content() { ?>
		<label>
...
class Bauer_Customize_Multicheck_Control extends WP_Customize_Control {
	public $description = '';
	public $subtitle = '';
...
// Sorter Control
class Bauer_Customize_Control_Sorter extends WP_Customize_Control {
	...
	public function render_content() { ?>
		<div class="bauer-sortable">
			...
			echo '<ul id="'. $this->id .'_sortable">'; ?>
				...
				foreach ( $values as $val ) :
					...
					if ( $label ) : ?>
						<li data-value="<?php echo esc_attr( $val ); ?>" class="bauer-sortable-li">
							<?php echo esc_html( $label ); ?>
							<span class="bauer-hide-sortee fa fa-toggle-on"></span>
						</li>
					<?php
					// End if label check
					endif;
				...
				foreach ( $choices as $val => $label ) { ?>
					<li data-value="<?php echo esc_attr( $val ); ?>" class="bauer-sortable-li bauer-hide">
						<?php echo esc_html( $label ); ?>
						<span class="bauer-hide-sortee fa fa-toggle-on fa-rotate-180"></span>
					</li>
				<?php } ?>
			</ul>
		</div><!-- .bauer-sortable -->
		...
		jQuery(document).ready( function($) {
			...
			sortableUl.on( "sortstop", function( event, ui ) {
				bauerUpdateSortableVal();
			} );

			// Toggle classes
			sortableUl.find( 'li' ).each( function() {
				$( this ).find( '.bauer-hide-sortee' ).on('click', function() {
					$( this ).toggleClass( 'fa-rotate-180' ).parents( 'li:eq(0)' ).toggleClass( 'bauer-hide' );
				} );
			})
			// Update Sortable when hidding/showing items
			$( '#<?php echo esc_html( $this->id ); ?>_sortable span.bauer-hide-sortee' ).on('click', function() {
				bauerUpdateSortableVal();
			} );
			// Used to update the sortable input value
			function bauerUpdateSortableVal() {
				var values = [];
				sortableUl.find( 'li' ).each( function() {
					if ( ! $( this ).hasClass( 'bauer-hide' ) ) {
...
class Bauer_Fonts_Dropdown_Custom_Control extends WP_Customize_Control {
	public function render_content() {
	$this_val = $this->value(); ?>
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			<select <?php $this->link(); ?> style="width:100%;">
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default', 'bauer' ); ?></option>
			<?php
			// Add custom fonts from child themes
			if ( function_exists( 'bauer_add_custom_fonts' ) ) {
				$fonts = bauer_add_custom_fonts();
				if ( $fonts && is_array( $fonts ) ) { ?>
					<optgroup label="<?php esc_html_e( 'Custom Fonts', 'bauer' ); ?>">
			...
			$std_fonts = '';
			if ( $std_fonts = bauer_standard_fonts() ) { ?>
				<optgroup label="<?php esc_html_e( 'Standard Fonts', 'bauer' ); ?>">
					<?php
			...
			// Google font options
			$google_fonts = '';
			if ( $google_fonts = bauer_google_fonts_array() ) { ?>
				<optgroup label="<?php esc_html_e( 'Google Fonts', 'bauer' ); ?>">
					<?php
					// Loop through font options and add to select
