...
// Start class
if ( ! class_exists( 'Bauer_Customizer' ) ) {
	class Bauer_Customizer {
		...
		public function __construct() {
			...
			$this->panels = array(
				'general' => array(
					'title' => esc_html__( 'General Options', 'bauer' ),
				),
				'topbar' => array(
					'title' => esc_html__( 'Top Bar', 'bauer' ),
				),
				'header' => array(
					'title' => esc_html__( 'Header', 'bauer' ),
				),
				'featuredtitle' => array(
					'title' => esc_html__( 'Featured Title', 'bauer' ),
				),
				'maincontent' => array(
					'title' => esc_html__( 'Main Content', 'bauer' ),
				),
				'layout' => array(
					'title' => esc_html__( 'Layout', 'bauer' ),
				),
				'blog' => array(
					'title' => esc_html__( 'Blog', 'bauer' ),
				),
				'blogsingle' => array(
					'title' => esc_html__( 'Blog Single Post', 'bauer' ),
				),
				'sidebarwidget' => array(
					'title' => esc_html__( 'Sidebar Widget', 'bauer' ),
				),
				'projects' => array(
					'title' => esc_html__( 'Project Related', 'bauer' ),
				),
				'shop' => array(
					'title' => esc_html__( 'Shop', 'bauer' ),
				),
				'footer' => array(
					'title' => esc_html__( 'Footer', 'bauer' ),
				),
				'footerwidget' => array(
					'title' => esc_html__( 'Footer Widget', 'bauer' ),
				),
				'bottombar' => array(
					'title' => esc_html__( 'Bottom Bar', 'bauer' ),
				),
			);
			...
			if ( ! is_admin() || is_customize_preview() ) {
				...
				if ( is_customize_preview() && $this->enable_postMessage ) {
					...
					add_action( 'bauer_custom_colors_css', array( $this, 'head_css' ), 999 );
				}
			}
		}
		...
		public function add_sections() {
			...
			// Apply filters
			$this->sections = apply_filters( 'bauer_customizer_sections', $this->sections );
			...
			$all_panels = $this->panels;
				...
				foreach( $all_panels as $id => $val ) {
					...
					if ( $is_section ) {
						...
						$wp_customize->add_section( 'bauer_'. $id, array(
							'priority' => $priority,
							'title'    => $title,
						) );
					...
					// Add Panel
						...
						$wp_customize->add_panel( 'bauer_'. $id, array(
							'priority' => $priority,
							'title'    => $title,
						) );
		...
		public function create_sections( $wp_customize ) {
			...
			foreach ( $this->sections as $section_id => $section ) {
				...
				foreach ( $settings as $setting ) {
					...
					} elseif ( 'bauer-heading' == $control_type ) {
						$control_object = 'Bauer_Customizer_Heading_Control';
					} elseif ( 'bauer-sortable' == $control_type ) {
						$control_object = 'Bauer_Customize_Control_Sorter';
					} elseif ( 'bauer_textarea' == $control_type ) {
						$control_object = 'Bauer_Customizer_Textarea_Control';
					} else {
		...
		public static function customize_controls_print_styles() { ?>
			 <style type="text/css">
				/* Icons */
				#accordion-panel-bauer_general > h3:before,
				#accordion-panel-bauer_typography > h3:before,
				#accordion-panel-bauer_topbar > h3:before,
				#accordion-panel-bauer_header > h3:before,
				#accordion-panel-bauer_featuredtitle > h3:before,
				#accordion-panel-bauer_maincontent > h3:before,
				#accordion-panel-bauer_layout > h3:before,
				#accordion-panel-bauer_blog > h3:before,
				#accordion-panel-bauer_blogsingle > h3:before,
				#accordion-panel-bauer_sidebarwidget > h3:before,
				#accordion-panel-bauer_projects > h3:before,
				#accordion-panel-bauer_shop > h3:before,
				#accordion-panel-bauer_footer > h3:before,
				#accordion-panel-bauer_footerwidget > h3:before,
				#accordion-panel-bauer_bottombar > h3:before { display: inline-block; width: 20px; height: 20px; font-size: 20px; font-style: normal; vertical-align: top; text-align: center;-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; color: #0085ba; margin-right: 6px; font-family: "dashicons"; content: "\f108"; }

				#accordion-panel-bauer_typography > h3:before { content: "\f215" }
				#accordion-panel-bauer_topbar > h3:before { content: "\f480" }
				#accordion-panel-bauer_header > h3:before { content: "\f175" }
				#accordion-panel-bauer_featuredtitle > h3:before { content: "\f209" }
				#accordion-panel-bauer_maincontent > h3:before { content: "\f134" }
				#accordion-panel-bauer_layout > h3:before { content: "\f535" }
				#accordion-panel-bauer_blog > h3:before { content: "\f109" }
				#accordion-panel-bauer_blogsingle > h3:before { content: "\f109" }
				#accordion-panel-bauer_sidebarwidget > h3:before { content: "\f135" }
				#accordion-panel-bauer_projects > h3:before { content: "\f481" }
				#accordion-panel-bauer_shop > h3:before { content: "\f174" }
				#accordion-panel-bauer_footerwidget > h3:before { content: "\f209" }
				#accordion-panel-bauer_footer > h3:before { content: "\f535" }
				#accordion-panel-bauer_footer > h3:before { content: "\f535" }
				#accordion-panel-bauer_bottombar > h3:before { content: "\f138" }
				
				/* General Tweaks */
				.customize-control-select select { min-width: 100% }

				/* Slider UI */
				li.customize-control.customize-control-bauer_slider_ui input[type=text] { width: 20%; float: left; text-align: center; }
				li.customize-control.customize-control-bauer_slider_ui .ui-slider-horizontal.bauer-slider-ui { float: right; width: 75%; height: 5px; margin-top: 10px; color: #414042; position: relative; border-radius: 5px; border: 1px solid #747474; border-bottom-color: #aeaeae; background-color: #cdcdcd; background: -webkit-linear-gradient(#aaaaaa, #cdcdcd); background: -o-linear-gradient(#aaaaaa, #cdcdcd); background: -moz-linear-gradient(#aaaaaa, #cdcdcd); background: linear-gradient(#aaaaaa, #cdcdcd); }
				li.customize-control.customize-control-bauer_slider_ui .ui-slider-horizontal .ui-slider-handle { position: absolute; z-index: 2; width: 17px; height: 17px; cursor: default; top: -7px; margin-left: -10px; border-radius: 50%; border: 1px solid #9e9e9e; -webkit-box-shadow: inset 0 1px 2px rgba(0,0,0,.07); box-shadow: inset 0 1px 2px rgba(0,0,0,.07); cursor: pointer; background-color: #f5f5f5; background: -webkit-linear-gradient(#f8f8f8, #ededed); background: -o-linear-gradient(#f8f8f8, #ededed); background: -moz-linear-gradient(#f8f8f8, #ededed); background: linear-gradient(#f8f8f8, #ededed); box-shadow: 0 2px 2px rgba(0,0,0,0.24); }

				/* Sortable */
				.bauer-sortable ul { margin-top: 10px }
				.bauer-sortable li.bauer-sortable-li { cursor: move; background: #fff; padding: 0; padding-left: 15px; height: 40px; line-height: 40px; white-space: nowrap; border: 1px solid #dfdfdf; text-overflow: ellipsis; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; color: #222; margin-bottom: 5px; margin-top: 0; overflow: hidden; position: relative; }
				.bauer-sortable li.bauer-sortable-li:hover { border-color: #999; z-index: 10; }

				.bauer-sortable li.bauer-sortable-li .fa { display: block; position: absolute; top: 0; right: 0; width: 42px; height: 40px; line-height: 40px; margin: 0; text-align: center; font-size: 18px; }

				.bauer-sortable li.bauer-sortable-li .fa:after {
				...
				.bauer-sortable li.bauer-sortable-li .fa:before {
					content: "";
					position: absolute;
					right: 6px;
					top: 6px;
				...
				.bauer-sortable li.bauer-sortable-li .fa:hover:before { border-color: #0085ba; }

				.bauer-sortable li.bauer-sortable-li .bauer-hide-sortee { cursor: pointer }
				.bauer-sortable ul li:last-child { margin-bottom: 0 }
				.bauer-sortable li.bauer-hide .fa:after { color: #dfdfdf; }
				.bauer-sortable li.bauer-hide { opacity: 0.65; }

				/* Custom Heading */
				.bauer-customizer-heading { color: #23282d; display: block; padding-top: 28px; padding-bottom: 5px; border-bottom: 2px solid #ddd; font-size: 16px; font-weight: 600; }
			 </style>
		...
		public function customize_preview_init() {
			if ( $this->enable_postMessage ) {
				wp_enqueue_script( 'bauer-customize-preview',
		...
		public function loop_through_inline_css( $return = 'css' ) {
			...
			foreach ( $settings as $settings_array ) {
				...
				foreach ( $settings_array as $setting ) {
					...
					$theme_mod  = bauer_get_mod( $setting_id, $default );
					...
					if ( apply_filters( 'bauer_remove_default_mods', false ) ) {
						$get_all_mods = bauer_get_mods();
					...
					// Sanitize data
					if ( $sanitize ) {
						$theme_mod = bauer_sanitize_data( $theme_mod, $sanitize );
					} else {
						$theme_mod = $theme_mod;
					}
