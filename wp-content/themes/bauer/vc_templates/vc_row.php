 ...
 */
$el_class = $img_position = $halfrow_image = $img_columns = $img_halfrow = $full_height = $full_width = $equal_height = $flex_row = $columns_placement = $content_placement = $parallax = $parallax_image = $css = $el_id = $video_bg = $video_bg_url = $video_bg_parallax = $fullwidth = $arrow = $scroll_id = $css_animation = $image_video = '';
$disable_element = '';
$output = $after_output = '';
...
$bauer_wrap_classes = 'vc-custom-col-spacing clearfix';

if ( ! empty( $column_spacing ) ) {
	$bauer_wrap_classes .= ' vc-col-spacing-'. $column_spacing;
}

if ( $img_halfrow == 'absolute' ) {
	$bauer_wrap_classes .= ' image-absolute';
}

$output .= '<div class="'. $bauer_wrap_classes .'">';
...
if ( $image_video )
	$video_icon = '<div class="bauer-video-icon clearfix white"><a class="icon-wrap popup-video" href="'. $image_video .'">play<span class="circle"></span><span class="circle-2"></span></a></div>';
...
if ( $fullwidth == 'yes' ) {
	...
	$output .= '<div class="bauer-container">';
	$output .= '<div class="row-inner clearfix">' .wpb_js_remove_wpautop( $content ) .'</div>';
	$output .= '</div>';
}
...
if ( $img_halfrow == 'parallax' ) {
	...
	$output .= '<div class="image-container img-parallax '. implode( ' ', $img_asiderow_attributes ) .'" style="top:'. $img_offset_top .'"><div class="inner" style="right:'. $img_offset_right .'"><img src="'. esc_url( $halfrow_image_src ) .'" data-parallax=\'{"x" : '. $px .', "y" : '. $py .'}\' alt="'. esc_attr__( 'Image', 'bauer' ) .'">'. $video_icon .'</div></div>';
}
...
if ( $img_halfrow == 'absolute' ) {
	...
	$output .= '<div class="image-container" style="'. $css .'"><img src="'. esc_url( $halfrow_image_src ) .'" alt="'. esc_attr__( 'Image', 'bauer' ) .'"></div>';
}

if ( $arrow == 'yes' )
	$output .= '<a href="#'. esc_attr( $scroll_id ) .'" class="scroll-btn"></a>';
