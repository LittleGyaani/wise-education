...
if ( $line_one == '#f35c27' ) { $cls1 .= ' accent';
} else { if ( $line_one ) $css1 = 'background-color:'. $line_one .';'; }

if ( $gradient && function_exists('bauer_hex2rgba') ) {
	$cls1 .= ' gradient';
	$css1 = 'background: '. bauer_hex2rgba($line_one, 1) .';background: -moz-linear-gradient(left, '. bauer_hex2rgba($line_one, 1) .' 0%, '. bauer_hex2rgba($line_one, 0.3) .' 100%);background: -webkit-linear-gradient( left, '. bauer_hex2rgba($line_one, 1) .' 0%, '. bauer_hex2rgba($line_one, 0.3) .' 100% );background: linear-gradient(to right, '. bauer_hex2rgba($line_one, 1) .' 0%, '. bauer_hex2rgba($line_one, 0.3) .' 100%);';
}
...
if ( $font_family != 'Default' ) {
	bauer_enqueue_google_font( $font_family );
	$css .= 'font-family:'. $font_family .';';
}

if ( $per_color == '#f35c27' ) { $pcls .= ' accent';
...
wp_enqueue_script( 'bauer-appear' );
printf( '
	<div class="bauer-progress clearfix %7$s" style="%6$s">%1$s %2$s
