...
$content_html = $text_css = $inner_css = $icon_offset = $icon_css = $icon_cls = $icon_html = $icon_data = $hover_html = $hover_css = $image_css = '';
...
if ( $desc_font_family != 'Default' ) {
	bauer_enqueue_google_font( $desc_font_family );
...
if ( $button_font_family != 'Default' ) {
	bauer_enqueue_google_font( $button_font_family );
	$button_css .= 'font-family:'. $button_font_family .';';
}

if ( $icon_display == 'icon-font' ) {
	$icon = bauer_get_icon_class( $atts, 'icon' );
	if ( $icon && $icon_type != '' ) {
...
if ( $url_style == 'button' && $link_url ) {
	...
	$content_html .= sprintf(
		'<div class="url-wrap" style="%6$s">
			<a target="%5$s" class="bauer-button %3$s" href="%2$s" style="%4$s" %7$s>%1$s</a>
		</div>',
		esc_html( $link_text ),
		esc_attr( $link_url ),
		$button_cls,
...
printf( '<div class="bauer-icon-box clearfix %1$s" style="%2$s" %5$s>%6$s<div class="wrap-inner" style="position: relative;">%3$s %4$s %6$s</div></div>',
	$cls,
	$css,
	$icon_html,
	$content_html,
