...
function register_testimonials_post_type() {
    ...
    $labels = array(
        ...
        'singular_name'      => esc_html__( 'Testimonials Item', 'bauer' ),
...
function testimonials_updated_messages( $messages ) {
    ...
    $messages['testimonials'] = array(
        ...
        1  => esc_html__( 'Testimonials updated.', 'bauer' ),
        2  => esc_html__( 'Custom field updated.', 'bauer' ),
        3  => esc_html__( 'Custom field deleted.', 'bauer' ),
        4  => esc_html__( 'Testimonials updated.', 'bauer' ),
        5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Testimonials restored to revision from %s', 'bauer' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
        6  => esc_html__( 'Testimonials published.', 'bauer' ),
        7  => esc_html__( 'Testimonials saved.', 'bauer' ),
        8  => esc_html__( 'Testimonials submitted.', 'bauer' ),
        9  => sprintf(
            esc_html__( 'Testimonials scheduled for: <strong>%1$s</strong>.', 'bauer' ),
            date_i18n( esc_html__( 'M j, Y @ G:i', 'bauer' ), strtotime( $post->post_date ) )
        ),
        10 => esc_html__( 'Testimonials draft updated.', 'bauer' )
    );
    return $messages;
}
...
function register_testimonials_taxonomy() {
    ...
    $labels = array(
        'name'                       => esc_html__( 'Testimonials Categories', 'bauer' ),
