...
class WPBakeryShortCode_Mungu_content_testimonial extends WPBakeryShortCode {
    protected function content( $atts, $content = null){
        ...
        if ($posts_query->have_posts()) {
            ...
            if ($theme_type == 'rehub') {
                $img = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'rehub-vc-ceo-team');
                $post_items .= "<div class='swiper-slide $theme_type'>
                    <div class='uk-grid-large uk-child-width-expand@m uk-grid'>
                        <div class='uk-grid-1-3'>
                            <div class='themeton-image' data-bg-image='".$img[0]."'>
                                <img class='head-img' src='".get_template_directory_uri()."/images/dim/2x3.png' alt='Ratio'/>
                            </div>
                        </div>
                        <div class='uk-inline'>
                        <div class='uk-position-small uk-position-center-left uk-overlay'>
                            <h3 class='info-title'>".Themeton_Std::getmeta('input_name') ."</h3>
                            <h5 class='mt0 color-ancient'>".Themeton_Std::getmeta('position')."</h5>
                            <p class='mb2'>$excerpt</p>
                            <div class='swiper-button-prev uk-icon uk-position-relative' data-uk-icon='icon: chevron-left; ratio: 2' style='left: auto; margin-left:-10px;'></div>
                            <div class='swiper-button-next uk-icon uk-position-relative' data-uk-icon='icon: chevron-right; ratio: 2'></div>
                        </div>
                        </div>
                    </div>
                </div>";
        ...
        if($theme_type == 'rehub') $result = "<div class='nxt-testimonial $theme_type uk-container $extra_class' $data>
                                        <div class='swiper-container testi-caraousel'>
                                            <div class='swiper-wrapper'>
                                                $item_result
                                            </div>
